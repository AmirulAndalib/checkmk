...
export function set_inpage_search_result_info(text: string) {
  ...
  if (result_with_row_info) {
    ...
    const [_old_text, old_count] = result_with_row_info.innerHTML.split(':')
    ...
    /* eslint-disable-next-line no-unsanitized/property -- Highlight existing violations CMK-17846 */
    result_with_row_info.innerHTML = new_text + ': ' + (Number(old_count) + Number(new_count))

    return
  }

  const row_info_div: HTMLElement | null = document.getElementById('row_info')
  const page_menu_popups_div: HTMLElement | null = document.getElementById('page_menu_popups')
...
export function get_url_param(name: string, url: string | undefined = undefined) {
...
export function makeuri(
  ...
  for (let i = 0; i < tmp.length; i++) {
    pair = tmp[i].split('=')
    if (pair[0][0] == '_' && pair[0] != '_username' && pair[0] != '_secret' && pair[0] != '_active')
      // Skip _<vars>
      continue
...
function get_clip_path_polygon(perc: number) {
  ...
  if (perc > 87.5) {
    ...
    return (
      'polygon(50% 50%, 50% 0, 100% 0, 100% ' + Math.floor(100 - ((perc - 62.5) / 25) * 100) + '%)'
    )
  } else if (perc > 37.5) {
    return (
...
export function toggle_more(trigger: HTMLElement, toggle_id: string, dom_levels_up: number) {
  event!.stopPropagation()
  let container: HTMLElement | ParentNode | null = document.getElementById(toggle_id)
  let state
...
export function set_focus_by_name(form_name: string | undefined, field_name: number) {
  ...
  set_focus(
    (document.getElementById('form_' + form_name) as HTMLFormElement).elements[
      field_name
    ] as HTMLElement | null
  )
}
...
export function update_pending_changes(changes_info: Nullable<string>, changes_tooltip: string) {
  if (!changes_info) {
    return
  }

  // Update container div CSS class and tooltip
  const page_state_div = document.getElementsByClassName('page_state')[0] as HTMLElement | null
  if (page_state_div) {
...
export function get_computed_style(object: null | undefined | Element, property: string) {
...
export function copy_to_clipboard(text: string, success_msg: string | null = null) {
  try {
...
export function copy_dom_element_content_to_clipboard(node_id: string, success_msg = '') {
...
export function querySelectorAllByClassName<T extends HTMLElement>(className: string) {
