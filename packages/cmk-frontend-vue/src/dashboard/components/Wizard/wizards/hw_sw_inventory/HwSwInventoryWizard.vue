...
const recapAndNext = () => {
  ...
  const inventoryPathLabel = inventoryPaths.value.find(
    (item) => item.name === inventoryPath.value
  )?.title
  if (!inventoryPathLabel) {
    validationError.value = _t('The selected HW/SW inventory property is invalid.')
    return
  }
