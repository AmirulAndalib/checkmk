...
<template>
  <div class="cmk-tag-match-component">
    <template v-for="index in numberOfRows" :key="index - 1">
      <div class="cmk-tag-match-component__group">
        <FormAutocompleter
      ...
      <div class="cmk-tag-match-component__operator">
        <CmkDropdown
      ...
      <div class="cmk-tag-match-component__value">
        <FormAutocompleter
      ...
      <div class="cmk-tag-match-component__clear">
        <button
          v-if="hasAnyValue(index - 1)"
          type="button"
          class="cmk-tag-match-component__clear-button"
          @click="clearRow(index - 1)"
        >
          <CmkIcon :aria-label="_t('Remove row')" name="close" size="xxsmall" />
        </button>
        <div v-else class="cmk-tag-match-component__clear-spacer"></div>
      </div>
    </template>
  </div>
</template>
...
<style scoped>
.cmk-tag-match-component {
  display: grid;
  grid-template-columns: 1fr auto 1fr auto;
  gap: var(--dimension-4);
}

.cmk-tag-match-component__group,
.cmk-tag-match-component__value {
  min-width: 0;
}

.cmk-tag-match-component__clear {
  display: flex;
  justify-content: center;
}

.cmk-tag-match-component__clear-button {
...
.cmk-tag-match-component__clear-spacer {
