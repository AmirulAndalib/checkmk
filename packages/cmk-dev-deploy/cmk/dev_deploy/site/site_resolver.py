...
from cmk.dev_deploy.state.deploy_state import STATE_BASE
...
def find_site_root(site_name: str | None) -> Path | None:
    ...
    if site_root.exists():
        return site_root
    # Site dir gone but deploy data (deploy state, version clone) may
    # still exist — return the path so teardown can clean up.
    state_dir = STATE_BASE / site_name
    if state_dir.exists() or (DEV_VERSIONS_DIR / site_name).exists():
        return site_root
    return None
