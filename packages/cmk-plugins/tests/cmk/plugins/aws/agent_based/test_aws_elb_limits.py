...
from cmk.plugins.aws.agent_based.aws_elb_limits import (
    check_aws_elb_limits,
    discover_aws_elb_limits,
)
...
def test_discover_aws_elb_limits(
    ...
    assert list(discover_aws_elb_limits(parsed_section)) == list(expected_discoveries)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [['[["load_balancers",', '"TITLE",', "10,", "1,", '"REGION"]]']],
            [
                Metric("aws_elb_load_balancers", 1.0),
...
def test_check_aws_elb_limits(
    ...
    parsed_section = parse_aws_limits_generic(info)
    assert list(check_aws_elb_limits(item, params, parsed_section)) == list(expected_results)
