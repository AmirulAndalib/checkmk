...
"""Bazel-invoked entry point that generates the OpenAPI spec.

The library functions in this module (`_make_spec`, `populate_spec`, `_redoc_spec`)
are also imported by spec-related unit tests.
"""
...
def _redoc_spec() -> ReDocSpec:
    return {
        "info": {
            "description": apispec_utils.dedent(_REDOC_API_DESCRIPTION)
            .strip()
