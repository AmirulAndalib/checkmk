...
def test_discover_cisco_prime_wifi_access_points() -> None:
    """Test discovery function for cisco_prime_wifi_access_points check."""
    parsed = parse_cisco_prime_wifi_access_points(_STRING_TABLE)
    result = list(discover_cisco_prime_wifi_access_points(parsed))
    assert result == [Service()]


def test_check_cisco_prime_wifi_access_points_no_levels() -> None:
    """Test check function for cisco_prime_wifi_access_points check without configured levels."""
    parsed = parse_cisco_prime_wifi_access_points(_STRING_TABLE)
    result = list(check_cisco_prime_wifi_access_points({}, parsed))
    assert result == [
        Result(state=State.OK, summary="Percent Critical: 25.00%"),
        Metric("ap_devices_percent_unhealthy", 25.0),
        Result(state=State.OK, summary="Cleared: 3"),
        Metric("ap_devices_cleared", 3),
        Result(state=State.OK, summary="Critical: 1"),
        Metric("ap_devices_critical", 1),
    ]


def test_check_cisco_prime_wifi_access_points_with_levels() -> None:
    """Test check function for cisco_prime_wifi_access_points check with configured levels."""
    parsed = parse_cisco_prime_wifi_access_points(_STRING_TABLE)
    result = list(check_cisco_prime_wifi_access_points({"levels": (20, 40)}, parsed))
    assert result == [
        Result(state=State.WARN, summary="Percent Critical: 25.00% (warn/crit at 20.00%/40.00%)"),
        Metric("ap_devices_percent_unhealthy", 25.0, levels=(20, 40)),
        Result(state=State.OK, summary="Cleared: 3"),
        Metric("ap_devices_cleared", 3),
        Result(state=State.OK, summary="Critical: 1"),
        Metric("ap_devices_critical", 1),
